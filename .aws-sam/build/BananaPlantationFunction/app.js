  ...
  "node_modules/yup/index.js"(exports2) {
    ...
    function getIn(schema2, path, value, context = value) {
      ...
      propertyExpr.forEach(path, (_part, isBracket, isArray) => {
        ...
        if (!isArray) {
          if (!schema2.fields || !schema2.fields[part]) throw new Error(`The schema does not contain the path: ${path}. (failed at: ${lastPartDebug} which is a type: "${schema2.type}")`);
          parent = value;
...
` + (formattedResult !== formattedValue ? `result of cast: ${formattedResult}` : ""));
      ...
      validate(value, options) {
        ...
        return new Promise((resolve, reject) => schema2._validate(value, options, (error, parsed) => {
    ...
    var Lazy = class _Lazy {
      constructor(builder) {
        ...
        this._resolve = (value, options = {}) => {
          ...
          if (this.spec.optional) schema2 = schema2.optional();
          return schema2.resolve(options);
        };
...
// functions/position/position.validator.ts
...
var schema = yup.object({
  start: yup.object({
    ...
    direction: yup.string().oneOf(["N", "S", "E", "W"], "Direction must be one of N, S, E, W")
  }),
  instructions: yup.string().matches(/^[AGD]+$/, "Instructions must only contain A, G, or D")
});
...
// functions/position/position.namespace.ts
var MOVES = {
  N: { x: 0, y: 1 },
  E: { x: 1, y: 0 },
  S: { x: 0, y: -1 },
...
// functions/position/position.service.ts
var getNextPosition = (actualPosition, instruction) => {
  if (instruction === "A" /* A */) {
    const { x, y, direction: direction2 } = actualPosition;
    const move = MOVES[direction2];
    const NewPosition = {
      ...actualPosition,
      x: x + move.x,
      y: y + move.y
  ...
  const rotation = instruction === "D" /* D */ ? 1 : -1;
...
var getResult = (actualPosition, instructions) => {
